...
<div class="sidebar-user">
    ...
    <div class="user-profile-avatar">
        <div class="info-new">
            ...
            <h5>{{$dataUser->getFirstLastName()}}</h5>
            <p>{{ __("Member Since :time",["time"=> date("M Y",strtotime($dataUser->created_at))]) }}</p>
            <h5 class=" align-items-center">
                {{$dataUser->getBusinessName()}}
                @if ($dataUser->isOwner())
                <i class="fa fa-key ml-2" style="color: #d2b960;"></i>
                @endif
            </h5>
    ...
    <div class="user-profile-plan">
        ...
        <a href=" {{ route("user.application.form") }}">{{ __("Become a vendor") }}</a>
        @endif
    </div>
