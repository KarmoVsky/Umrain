    ...
    <h2 class="title-bar no-border-bottom">
        ...
        @if($row->id)
            <div class="title-action">
                @if(Auth::user()->hasPermission('hotel_room_manage'))
        ...
        <form action="{{route('hotel.vendor.store',['id'=>($row->id) ? $row->id : '-1','lang'=>request()->query('lang')])}}" method="post">
            ...
            <div class="form-add-service">
                <div class="nav nav-tabs nav-fill" id="nav-tab" role="tablist">
                    @if(Auth::user()->hasPermission('hotel_manage_others'))
                        <a data-toggle="tab" href="#nav-tour-content" aria-selected="true"
                           class="{{ Auth::user()->hasPermission('hotel_manage_others') ? 'active' : '' }}">
                            {{__("1. Content")}}
                        </a>
                    ...
                    @if(is_default_lang())
                        ...
                        @if(Auth::user()->hasPermission('hotel_manage_attributes'))
                ...
                <div class="tab-content" id="nav-tabContent">
                    @if(Auth::user()->hasPermission('hotel_manage_others'))
                        <div class="tab-pane fade {{ Auth::user()->hasPermission('hotel_manage_others') ? 'show active' : '' }}" id="nav-tour-content">
                    ...
                    @endif
                    @if(is_default_lang())
                        <div class="tab-pane fade {{ !Auth::user()->hasPermission('hotel_manage_others') ? 'show active' : '' }}" id="nav-tour-pricing">
