...
class MenuWalker
{
    ...
    public function getMenuNames()
    {
        ...
        $names = [];
        if (!empty($items)) {
            $names = $this->extractNames($items);
        }
        return $names;
    }

    // دالة لاستخراج الأسماء من العناصر
    protected function extractNames($items)
    {
        $names = [];
        foreach ($items as $item) {
            $names[] = $item['name']; // إضافة اسم العنصر
            if (!empty($item['children'])) {
                $names = array_merge($names, $this->extractNames($item['children'])); // إضافة الأسماء الفرعية
            }
        }
        return $names;
    }
