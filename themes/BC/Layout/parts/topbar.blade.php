                ...
                @if(!Auth::check())
                    ...
                    @else
                        ...
                        <li class="login-item dropdown">
                            <a href="#" data-toggle="dropdown" class="login">{{__("Hello, :name",['name'=>Auth::user()->getfirstLastName(lastName: false)])}}
                                <i class="fa fa-angle-down"></i>
                            </a>
                            <ul class="dropdown-menu dropdown-menu-user text-left">

                                @if(($relatedbusinesses = Auth::user()->relatedBusinesses()) && $relatedbusinesses->isNotEmpty())
                                  @foreach ($relatedbusinesses as $relatedbusiness)
                                     ...
                                     <li class="menu-hr">
                                        <a class="d-flex justify-content-between"
                                           ...
                                           href="{{ $relatedbusiness->business->business_name != Auth::user()->getBusinessName() ? route('switch.business', ['id' => $relatedbusiness->business?->id]) : '#' }}"
                                           style="{{ $relatedbusiness->business->business_name == Auth::user()->getBusinessName() ? 'pointer-events: none; opacity: 0.6;' : '' }}">

                                            <img src="{{ $relatedbusiness->business?->avatar_id
                                            ? \Modules\Media\Helpers\FileHelper::url($relatedbusiness->business->avatar_id)
                                            : 'https://ui-avatars.com/api/?name=' . urlencode($relatedbusiness->business->business_name) . '&size=100&background=random' }}"
                                            width="20" height="20" class="rounded-circle">

                                            <span class="mx-2">{{ $relatedbusiness->business->business_name }}</span>
                                            <i class="fa fa-refresh" style="visibility: {{ $relatedbusiness->business->business_name != Auth::user()->getBusinessName() ? 'visible' : 'hidden' }}"></i>
                                        </a>
                                    </li>
                                ...
                                @if(($relatedbusinesses = Auth::user()->relatedBusinesses()) && $relatedbusinesses->count()>0)
                                  <li class="menu-hr">
                                      <a href="{{ route('leave.business', ['count'=>$relatedbusinesses->count()]) }}"><i class="fa fa-power-off"></i>
                                        {{__("Leave  :name",['name'=>Auth::user()->getBusinessName()])}} </a>
                                   </li>
                                @endif
                            ...
                            <form id="logout-form-topbar" action="{{ route('logout') }}" method="POST" style="display: none;">
                                {{-- @php Session::forget('active_business_id'); @endphp --}}
                                {{ csrf_field() }}
                            </form>
                        </li>
                    @endif
