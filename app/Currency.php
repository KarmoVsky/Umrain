...
class Currency
{
    ...
    public static function format($price,$main_currency = false, $override_format = null)
    {
        ...
        if (!empty($currency)) {
            ...
            if($override_format == 'custom') {
                // This section is used privatly for formatting prices on the hotels page.
                $dir = (app()->getLocale() == 'en')? 'ltr' : 'rtl';

                switch ($currency_format) {
                    case "right":
                        return span($s .$currency['symbol']);
                        break;
                    case "left_space":
                        return span($s . ' ' . $currency['symbol'], 'rtl');
                        break;
                    case "right_space":
                        return span($s . ' ' . $currency['symbol']);
                        break;
                    default:
                        return span(span($currency['symbol']) . span($s));
                        break;
                }
            } else {
                switch ($currency_format) {
