...
function updateServicesTable() {
    $services = ['hotel','car','tour','flight','space','event','boat'];
    DB::statement('SET FOREIGN_KEY_CHECKS = 0');
    DB::table('anisth_services')->truncate();
    DB::table('anisth_business_service')->truncate();
    DB::statement('SET FOREIGN_KEY_CHECKS = 1');
    if(!empty($services)) {
        foreach($services as $name) {
            $status = setting_item($name.'_disable');
            DB::table('anisth_services')->insert([
                'name' => $name,
...
function format_money($price, $override_format = null){

   return Currency::format((float)$price, false, $override_format);

}
function span($data, $dir = 'ltr') {
    return "<span dir='$dir' style='font-size: 18px; color: #1a2b48'>$data</span>";
}
