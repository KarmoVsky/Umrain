...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    public function getDisplayName($email = false)
    {
        ...
        /* if ($activeBusinessId) {
            ...
            $name = $business?->business_name ?? 'No Business Found';
        } */
    ...
    public function getBusinessName()
    {
        ...
        if ($activeBusinessId) {
            $business = Business::find($activeBusinessId);
            $name = $business?->business_name ?? 'No Business Name';
        }
        return $name;

    }

    public function getfirstLastName($email = false,$lastName = true)
    {
        $name = $this->name ?? "";
        if ((!empty($this->first_name) or !empty($this->last_name)) && $lastName) {
            $name = implode(' ', [$this->first_name,' ',$this->last_name]);
        } elseif ((!empty($this->first_name) or !empty($this->last_name)) && !$lastName) {
...
//                to admin
    ...
    public function business() {
        return $this->hasOne(\App\Models\Business::class, 'create_user');
    }

    public function businessRelations()
    {
        return $this->hasMany(\App\Models\BusinessRelation::class, 'user_id');
    }

    /**
     * Retrieves the related businesses for the user.
     *
     * This function fetches all business relations for the user, including the associated
     * business data. It then filters the results to ensure uniqueness based on the business name
     * and removes any null values.
     *
     * @return \Illuminate\Support\Collection A collection of unique, non-null related businesses
     */
    ...
    public function relatedBusinesses()
{
    return $this->businessRelations()
        ->with('business')
        ->get()
        ->filter(fn ($relation) => !is_null($relation->business)) // Remove null businesses
        ->unique(fn ($relation) => $relation->business->business_name ?? null);
}
    public function isOwner()
    {
        $isOwner=Business::where('id',session('active_business_id'))->where('create_user',$this->id)->exists();
        return $isOwner;
    }
