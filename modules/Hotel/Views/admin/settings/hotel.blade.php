            ...
            <div class="panel-body">
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        @php
                        $attributeKeys = [
                            'hotel_attribute_show_in_listing_page' => __('Which attribute show in listing page?'),
                            'hotel_distance_attribute' => __('Distance Attribute'),
                            'hotel_area_attribute' => __('Area Attribute'),
                        ];
                    @endphp

                    @foreach ($attributeKeys as $key => $label)
                        <div class="col-md-6">
                            <div class="form-group" data-condition="hotel_layout_item_search:is(list)">
                                <label class="">{{ $label }}</label>
                                <div class="form-controls">
                                    <?php
                                    $selectedAttribute = !empty($settings[$key]) ? \Modules\Core\Models\Attributes::find($settings[$key]) : false;
                                    ...
                                    \App\Helpers\AdminForm::select2(
                                        ...
                                        [
                                            'configs' => [
                                                ...
                                                'allowClear' => true,
                                                'placeholder' => __('Select an option or None'),
                                            ],
                                        ],
                                        $selectedAttribute ? [$selectedAttribute->id, $selectedAttribute->name] : null
                    ...
                    <div class="form-group">
                        ...
                        <div class="form-controls">
                            <select name="hotel_layout_map_option" class="form-control">
                                <option
                                    {{ (setting_item_with_lang('hotel_layout_map_option', request()->query('lang')) ?? '') == 'map_left' ? 'selected' : '' }}
                                    value="map_left">{{ __('Map Left') }}</option>
                                <option
                                    {{ (setting_item_with_lang('hotel_layout_map_option', request()->query('lang')) ?? '') == 'map_right' ? 'selected' : '' }}
                                    value="map_right">{{ __('Map Right') }}</option>
                            </select>
                        </div>
                    </div>
                    <div class="row">
...
@if(is_default_lang())
    ...
    <div class="row">
        <div class="col-sm-4">
            <h3 class="form-group-title">{{__("Customer Options")}}</h3>
            <p class="form-group-desc">{{__('customer config for hotel')}}</p>
        </div>
        <div class="col-sm-8">
            <div class="panel">
                <div class="panel-body">
                    <div class="form-group">
                        <label class="" >{{__("Allow customer can change their booking status")}}</label>
                        <div class="form-controls">
                            <label><input type="checkbox" name="hotel_allow_customer_can_change_their_booking_status" value="1" @if(!empty($settings['hotel_allow_customer_can_change_their_booking_status'])) checked @endif /> {{__("Yes please")}} </label>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="panel-body">
                <div class="form-group">
                    ...
                    <div class="mt-2 mb-3">
                        ...
                        @php $row = \App\Models\Module::where('name', $group)->first(); @endphp
                        <div class="items">
                            <?php
                                if(request()->query('lang')) {
                                    ...
                                    if(in_array($lang, $locales)) {
                                        ...
                                        $key_lang = "_".setting_item('site_locale');
                                    }
                                } else {
                                    $key_lang = "_".setting_item('site_locale');
                                }
                            ...
                            ?>
                            <div class="g-lang">
                                <textarea name="disable_message[name{{$key_lang}}]" class="form-control" placeholder="{{__('Maintenance message')}} ">@if(!empty($message)){{$message['name'.$key_lang]??null}}@endif</textarea>
                            </div><br>
                        </div>
                    </div>
                    </div>
