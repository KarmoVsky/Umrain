    ...
    <div class="container-fluid">
        <div class="d-flex justify-content-between mb20">
            ...
            <div class="title-actions">
                @if(empty($recovery))
                <a href="{{route('hotel.admin.create')}}" class="btn btn-primary">{{__("Create new hotel")}}</a>
                @endif
            </div>
        </div>
                    ...
                    @if(!empty($rows) and $hotel_manage_others)
                        ...
                        <div class="ml-3 position-relative">
                            ...
                            <div class="dropdown-menu px-3 py-3 dropdown-menu-right" aria-labelledby="dropdown_filters">
                                @include("Core::admin.global.advanced-filter-hotel")
                            </div>
                        </div>
                    @endif
                        ...
                        @if($rows->total() > 0)
                            @foreach($rows as $row)
                                <tr class="{{$row->status}}">
                                    ...
                                    <td>
                                        @if(($count = $row->vendorRequests->count()) > 0)
                                        @php
                                        $vendors = $row->vendorRequests;
                                        $names = [];
                                        foreach ($vendors as $vendor) {
                                            $names[] = $vendor->business->business_name;
                                        }
                                        $tooltipText = implode('<br>', $names);
                                        @endphp
                                        <span class="badge badge-warning" style="min-width:17px !important"
                                        data-toggle="tooltip"
                                        title="{!! $tooltipText !!}"
                                        ...
                                        >{{$count}}</span>
                                        @endif
                                        @if(($count = $row->acceptedVendors->unique('business_id')->count()) > 0)
                                        @php
                                        $vendors = $row->acceptedVendors;
                                        $names = [];
                                        foreach ($vendors as $vendor) {
                                            $names[] = $vendor->business->business_name;
                                        }
                                        ...
                                        $tooltipText = implode('<br>', $names);
                                        @endphp
                                        <span class="badge badge-success" style="min-width:17px !important"
                                        data-toggle="tooltip"
                                        title="{!! $tooltipText !!}"
                                        data-placement="bottom",
                                        data-html="true"
