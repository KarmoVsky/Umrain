...
//                    }
    ...
    public function store(Request $request,$hotel_id){
        ...
        $max_rooms = $room->number;


        $bookings = $room->getBookingsInRange($request->input('start_date'),$request->input('end_date'));
        $period = periodDate($request->input('start_date'),$request->input('end_date'));

        foreach ($period as $dt) {
            if(!empty($bookings))
                foreach($bookings as $booking) {
                    $booking_periods = periodDate($booking->start_date,$booking->end_date,false);
                    foreach ($booking_periods as $key=>$booking_period) {
                        if($booking_period->format('Y-m-d') === $dt->format('Y-m-d')) {
                            if($postData['number'] > ($max_rooms - $booking->number)) {
                                $message = __("Invalid Room Number Max: ")  . $max_rooms - $booking->number;
                                return $this->sendError( $message);
                            }
                            $postData['number']+=$booking->number;
                        }
                    }
                }
            if($postData['number'] < 0 || $postData['number'] > $max_rooms) {
                $message = __("Invalid Room Number Max: ")  . $max_rooms;
                return $this->sendError( $message);
            }
