...
class Hotel extends Bookable
{
    ...
    public    $invoice_new_booking_file             = 'Hotel::invoices.new_booking_detail';
    protected $fillable      = [
        'title',
        'content',
        'status',
    ...
    public $hotelAdultCapacity = 0;
    public $hotelChildrenCapacity = 0;
    ...
    public function termsByAttribute($settingKey)
    {
        $attributeId = setting_item($settingKey, 0);
        ...
        return $this->hasManyThrough(
            ...
            'id',
            'term_id'
        )->where('bravo_terms.attr_id', $attributeId)->with(['translation']);
    }


    public function getAttributeBySettingKey($settingKey)
    {
        $attributeId = setting_item($settingKey, 0);
        return $this->attributeClass::find($attributeId) ?? false;
    }
    ...
    public function getDetailUrl($include_param = true)
    {
        ...
        if ($include_param) {
            ...
            if (!empty($start = request()->input('start'))) {
                ...
                $param['start'] = $startDate ? $startDate->format('Y-m-d') : "";
            }

            if (!empty($end = request()->input('end'))) {
                $endDate = \DateTime::createFromFormat('m/d/Y', trim($end));
                $param['end'] = $endDate ? $endDate->format('Y-m-d') : "";
            }
        }
    ...
    public function addToCart(Request $request)
    {
        ...
        if (!empty($this->tmp_selected_rooms)) {
            foreach ($this->tmp_selected_rooms as $room) {
                if (isset($this->tmp_rooms_by_id[$room['id']])) {
                    $roomPrice = $this->tmp_rooms_by_id[$room['id']]->tmp_price; // سعر الغرفة
                    $roomCount = $room['number_selected']; // عدد الغرف المحددة
                    $total += $roomPrice * $roomCount;
                    $total_room_selected += $roomCount;
                    $total += $room['commissionperdayandperson'];

                    $selectedRoomsInfo[] = [
                        'room_id' => $room['id'],
                        'room_price' => $roomPrice,
                        'number_selected' => $roomCount,
                        'total_price_for_room' => $roomPrice * $roomCount
                    ];
                }
            }
        }
        ...
        $booking->calculateCommission($selectedRoomsInfo, $start_date, $end_date);
        ...
        $amount_check = $this->checkMinimumAmountAndMinimumGuests($total, $total_guests);
        if ($amount_check !== true) return $amount_check;
        $check = $booking->save();
                ...
                foreach ($this->tmp_selected_rooms as $room) {
                    if (isset($this->tmp_rooms_by_id[$room['id']])) {
                        ...
                        $hotelRoomBooking->fillByAttr([
                            ...
                            'price'      => $this->tmp_rooms_by_id[$room['id']]->tmp_price + ($room['commissionperdayandperson'] / max(1, $room['number_selected']))
                        ]);
    ...
    protected function checkMinimumAmountAndMinimumGuests($total, $total_guests)
    {
        $errors = [];

        $guestsConditionValid = true;
        if ($total_guests < $this->minimum_guests) {
            $errors['guests'] = __("The minimum number of guests is :min", ['min' => $this->minimum_guests]);
            $guestsConditionValid = false;
        }

        $amountConditionValid = true;
        if ($total < $this->minimum_amount) {
            $errors['amount'] = __("The total booking amount must be at least :min", ['min' => $this->minimum_amount]);
            $amountConditionValid = false;
        }
        if ($this->condition_type === 'on') {
            if (!$guestsConditionValid && !$amountConditionValid) {
                return $this->sendError(__("You must meet both conditions:") . "<br>" . implode("<br>", $errors));
            } elseif (!$guestsConditionValid) {
                return $this->sendError($errors['guests']);
            } elseif (!$amountConditionValid) {
                return $this->sendError($errors['amount']);
            }
        } elseif ($this->condition_type === null) {
            if (!$guestsConditionValid && !$amountConditionValid) {

                return $this->sendError(__("You must meet at least one of the conditions:") . "<br>" . implode("<br>", $errors));
            } elseif (!$guestsConditionValid && $this->minimum_amount === null) {
                return $this->sendError($errors['guests']);
            } elseif (!$amountConditionValid && $this->minimum_guests === null) {
                return $this->sendError($errors['amount']);
            }
        }
        return true;
    ...
    public function beforeCheckout(Request $request, $booking)
    {
        ...
        if (isset($filters['start_date']) && isset($filters['end_date'])) {
            foreach ($rooms as $room) {
                // Calculate the total available capacity for adults and children
                $this->hotelAdultCapacity += $room->number * $room->adults;
                $this->hotelChildrenCapacity += $room->number * $room->children;
                $bookedRoomsInRange = $room->getBookingsInRange($filters['start_date'], $filters['end_date']);
                $reservedAdultCapacity = 0;
                $reservedChildrenCapacity = 0;
                // Calculate reserved capacity for adults and children
                foreach ($bookedRoomsInRange as $row) {
                    $reservedAdultCapacity += $row->number * $room->adults;
                    $reservedChildrenCapacity += $row->number * $room->children;
                }
                $this->hotelAdultCapacity -= $reservedAdultCapacity;
                $this->hotelChildrenCapacity -= $reservedChildrenCapacity;
            }
            $filters['hotel_adult_capacity'] = $this->hotelAdultCapacity;
            $filters['hotel_children_capacity'] = $this->hotelChildrenCapacity;
        }
    ...
    public function getRoomsAvailability($filters = [])
    {
        ...
        $res = [];
        $this->tmp_rooms = [];
        $startDate = isset($filters['start_date']) ? strtotime($filters['start_date']) : null;
        $endDate = isset($filters['end_date']) ? strtotime($filters['end_date']) : null;
        $numberDays = 0;

        if ($startDate && $endDate) {
            $numberDays = max(1, ($endDate - $startDate) / 86400);
        }

        $adults = isset($filters['adults']) ? (int) $filters['adults'] : 1;

        $children = isset($filters['children']) ? (int) $filters['children'] : 0;
        ...
        foreach ($rooms as $room) {
            if ($room->isAvailableAt($filters)) {
                ...
                $commission = $booking->CalculateCommissionAddition($room->tmp_price, $totalGuests, $numberDays, $authorId, $hotelId, $room->id, $filters['start_date'], $filters['end_date']);
                $room->tmp_price = $room->tmp_price + $commission;
                $commissionperdayandperson = $booking->CalculateCommissionAdditionperdayandperson($room->tmp_price, $totalGuests, $numberDays, $authorId, $hotelId, $room->id, $filters['start_date'], $filters['end_date']);
                ...
                $res[] = [
                    'commissionperdayandperson'      => $commissionperdayandperson,
                    'id'              => $room->id,
                    ...
                    'price_html'      => format_money($room->tmp_price + $commissionperdayandperson, 'custom') . '<span class="unit">/' . ($room->tmp_nights ? __(':count nights', ['count' => $room->tmp_nights]) : __(":count night", ['count' => $room->tmp_nights])) . '</span>',
    ...
    public function search($request)
    {
        ...
        // $model_hotel->whereHas('rooms', function ($query) {
        //     $query->where('status', 'publish');
        // });
        $model_hotel->whereNotNull("bravo_hotels.location_id");
        ...
        if (!empty($request['start']) && !empty($request['end'])) {
            ...
            $model_hotel->whereHas('rooms', function ($query) use ($start_date, $end_date) {
                $query->where(function ($q) use ($start_date, $end_date) {
                    $q->whereDoesntHave('bookings', function ($subQuery) use ($start_date, $end_date) {
                        $subQuery->where(function ($dateQuery) use ($start_date, $end_date) {
                            $dateQuery->where('start_date', '<', $end_date)
    ...
    public function shouldDisplayVendorInfo()
    {
        $vendor = $this->vendor;

        $is_visible = $vendor->is_visible ?? 0;

        if ($is_visible && $this->vendor_override_show_name === 'show') {
            return true;
        }

        if (!$is_visible && $this->vendor_override_show_name === 'hidden') {
            return false;
        }
        ...
        if ($is_visible && $this->vendor_override_show_name === 'by_default') {
            return true;
        }
    ...
    {
        return $this->hasMany(\App\Models\BusinessRelation::class, 'service_id')
            ->where('service_type', 'hotel')->where('status', 'pending');
    }

    public function vendors()
    {
        return $this->hasMany(\App\Models\BusinessRelation::class, 'service_id')
            ->where('service_type', 'hotel')->where('status', 'approved');
    }

    public function acceptedVendors()
    {
        return $this->hasMany(\App\Models\BusinessRelation::class, 'service_id')
            ->where('service_type', 'hotel')->whereIn('status', ['approved', 'draft']);
    }

    public function allHotelRequests()
    {
        return $this->hasMany(\App\Models\BusinessRelation::class, 'service_id')
