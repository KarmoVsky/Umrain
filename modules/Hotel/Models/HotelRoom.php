...
class HotelRoom extends Bookable
{
    ...
    public function isAvailableAt($filters = []){
        ...
        //Adult - Children
        // This condition added by Anisth for task #55
        if(!empty($filters['hotel_adult_capacity']) && isset($filters['hotel_children_capacity'])) {
            $adults = (int)$filters['adults'];
            $adultCapacity = (int)$filters['hotel_adult_capacity'];

            $children = (int)$filters['children'];
            $childrenCapacity = (int)$filters['hotel_children_capacity'];

            if(!empty($filters['adults']) and $adultCapacity >= $adults && isset($filters['children']) && $filters['children'] !== "" && $childrenCapacity >= $children){
    ...
    {
        return $this->hasMany(HotelRoomBooking::class, 'room_id');
    }
}
