...
class UserController extends AdminController
{
    ...
    public function store(Request $request, $id)
    {
        ...
        $rules = [
            ...
            'business_name'             => 'nullable|max:255', //change
        ...
        $data = [
            ...
            'vendor_commission_calculate_way'=>$request->input('vendor_commission_calculate_way'),
            'vendor_commission_calculate_time'=>$request->input('vendor_commission_calculate_time'),
            'per_person'=>$request->input('per_person'),
            ...
            if (is_array($selected)) {
                ...
                if(!empty($res)){
                    foreach ($res as $item){
                        $items[] = [
                            'id'=>$item->id,
                            'text'=>$item->getDisplayName() ? $item->getDisplayName() . ' (#' . ($item->is_visible ? 'visible' : 'not visible') . ')' . ')' : $item->email . ' (#' . ($item->is_visible ? 'visible' : 'not visible') . ')'
                        ];
                    }
                }
            } else {
    ...
    public function getForSelect2_hotel(Request $request)
    {
        $pre_selected = $request->query('pre_selected');
        $selected = $request->query('selected');
        ...
        if ($pre_selected && $selected) {
            if (is_array($selected)) {
                $res = \DB::table('anisth_business_relations')
                    ->join('anisth_business', 'anisth_business_relations.business_id', '=', 'anisth_business.id')
                    ->whereIn('anisth_business_relations.id', $selected)
                    ->select('anisth_business_relations.id', 'anisth_business.business_name as business_name')
                    ->get();
                ...
                foreach ($res as $item) {
                    $items[] = [
                        'id' => $item->id,
                        'text' => $item->business_name?: __('No Name Available'),
                    ];
                }
                ...
                $item = \DB::table('anisth_business_relations')
                    ->join('anisth_business', 'anisth_business_relations.business_id', '=', 'anisth_business.id')
                    ->where('anisth_business_relations.id', 23)
                    ->select('anisth_business_relations.id', 'anisth_business.business_name as business_name')
                    ->first();
        ...
        $q = $request->query('q');
        $query = \DB::table('anisth_business_relations')
            ->join('anisth_business', 'anisth_business_relations.business_id', '=', 'anisth_business.id')
            ->where('anisth_business_relations.service_type', '=', 'hotel');

        if ($q) {
            $query->where(function ($query) use ($q) {
                $query->where('anisth_business.business_name', 'like', '%' . $q . '%')
                    ->orWhere('anisth_business_relations.id', $q);
            });
        }

        $res = $query
            ->select('anisth_business.id', 'anisth_business.business_name as business_name')
            ->groupBy('anisth_business.business_name')
            ->orderBy('anisth_business.business_name', 'asc')
            ->limit(100)
            ->get();
