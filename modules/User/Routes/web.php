...
Route::group(['prefix' => 'user', 'middleware' => ['auth', 'verified']], function () {
    ...
    Route::prefix('vendors')->middleware('has_role:user_view')->group(function () {
        Route::get('/', 'ManageUserController@index')->name("user.vendor.index");
        Route::get('/details/{id}', 'ManageUserController@userDetails')->name('user.vendor.details');
        Route::get('/bulkEdit', 'ManageUserController@bulkEditUser')->name("user.vendor.bulkEdit");
        Route::get('/delete-user', 'ManageUserController@deleteRelatedUser')->name('user.vendor.delete');
        Route::get('/search','ManageUserController@searchUser')->name('user.vendor.search');
        Route::post('/inviteUser','ManageUserController@inviteUser')->name('user.vendor.invite');
        Route::post('/update-user-services', 'ManageUserController@updateUserServices')->name('user.vendor.update');

        Route::get('/leave-business', 'ManageUserController@leaveBusiness')->name('leave.business'); ////////////

        Route::get('/switch-business', 'ManageUserController@switchBusiness')->name('switch.business'); /////////

        Route::get('/setOwner', 'ManageUserController@updateBusinessOwner')->name('user.vendor.setowner'); ////////
    ...
    Route::get('/business', 'UserController@business')->name("user.business.index");
    Route::post('/business', 'UserController@businessUpdate')->name("user.business.update");
    Route::get('/booking-history', 'UserController@bookingHistory')->name("user.booking_history");
    Route::get('/booking-report/bulkEdit/{id}', 'UserController@changeBookingStatusByCustomer')->name("hotel.customer.booking_history.bulk_edit");
    ...
    Route::get('/application-form', 'UserController@showApplicationForm')->name('user.application.form');
    Route::post('/application-form', 'UserController@submitApplicationForm')->name('user.application.submit');
