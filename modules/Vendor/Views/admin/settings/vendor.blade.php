                    ...
                    <div class="form-group" data-condition="vendor_enable:is(1)">
                        ...
                        <div class="form-controls">
                            <select name="vendor_commission_calculate_way" class="form-control">
                                <option value="addition" {{($settings['vendor_commission_calculate_way'] ?? '') == 'addition' ? 'selected' : ''  }}>{{__('Addition')}}</option>
                                <option value="dedict" {{($settings['vendor_commission_calculate_way'] ?? '') == 'dedict' ? 'selected' : ''  }}>{{__('Dedict')}}</option>
                            </select>
                        </div>
                    </div>
                    <div class="form-group" data-condition="vendor_enable:is(1)">
                        ...
                        <div class="form-controls">
                            <select name="vendor_commission_calculate_time" class="form-control">
                                <option value="one-time" {{($settings['vendor_commission_calculate_time'] ?? '') == 'one-time' ? 'selected' : ''  }}>{{__('One-time')}}</option>
                                <option value="per-day" {{($settings['vendor_commission_calculate_time'] ?? '') == 'per-day' ? 'selected' : ''  }}>{{__('Per-day')}}</option>
                            </select>
                        </div>
                    </div>

                    <div class="form-controls" data-condition="vendor_enable:is(1),vendor_commission_type:is(amount)">
                        <div class="form-group">
                            <label> <input type="checkbox" @if($settings['per_person'] ?? '' == 1) checked @endif name="per_person" value="1"> {{__("Per Person")}}</label>
                        </div>
                    </div>
                    <div class="form-group">
                        <label><strong>{{__("Vendor Commission Availability Type")}}</strong></label>
                        <div class="form-controls">
                                <div class="form-group-item">
                                    <div class="g-items-header">
                                        <div class="row">
                                            <div class="col-md-6">{{__("Availability Target (%)")}}</div>
                                            <div class="col-md-5">{{__('Vendor Commission Value')}}</div>
                                            <div class="col-md-1"></div>
                                        </div>
                                    </div>
                                    <div class="g-items">
                                        <?php
                                        $items = json_decode(setting_item('vendor_commission_availability_type'), true);
                                        if (json_last_error() !== JSON_ERROR_NONE || !is_array($items)) {
                                            $items = [];
                                        }
                                        $items = array_values($items);
                                        setting_item('vendor_commission_availability_type', json_encode($items));
                                                ...
                                                <div class="row">
                                                    <div class="col-md-6">
                                                        <input type="number" min="0" max="100" step="0.01"
                                                                name="vendor_commission_availability_type[{{$key}}][availability_target]"
                                                                class="form-control"
                                                                placeholder="{{__('Enter Availability Target (%)')}}"
                                                                value="{{ e($item['availability_target']) }}">
                                                    </div>
                                                    <div class="col-md-5">
                                                        <input type="number" min="0" step="0.01"
                                                                name="vendor_commission_availability_type[{{$key}}][vendor_commission_value]"
                                                                class="form-control"
                                                                placeholder="{{__('Enter Commission Value')}}"
                                                                value="{{ e($item['vendor_commission_value']) }}">
                                                    </div>
                                    ...
                                    <div class="g-more hide">
                                        <div class="item" data-number="__number__">
                                            <div class="row">
                                                <div class="col-md-6">
                                                    <input type="number" min="0" max="100" step="0.01"
                                                            __name__="vendor_commission_availability_type[__number__][availability_target]"
                                                            class="form-control"
                                                            placeholder="{{__('Enter Availability Target (%)')}}" value="">
                                                </div>
                                                <div class="col-md-5">
                                                    <input type="number" min="0" step="0.01"
                                                            __name__="vendor_commission_availability_type[__number__][vendor_commission_value]"
                                                            class="form-control"
                                                            placeholder="{{__('Enter Commission Value')}}" value="">
                                                </div>
