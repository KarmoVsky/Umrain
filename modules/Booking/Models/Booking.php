...
class Booking extends BaseModel
{
    ...
    private $commissionTime;
    private $commissionWay;
    private $perPerson;
    ...
    public function sendNewBookingEmails()
    {
        try {
            ...
            // to Vendor
            Mail::to(User::find($vendorId_id))->send(new NewBookingEmail($this, 'vendor'));
    ...
    public function sendStatusUpdatedEmails(){
        ...
        try{
            ...
            // to Vendor
            Mail::to(User::find($vendorId_id))->send(new StatusUpdatedEmail($this,'vendor'));
    ...
    public function getAvailableRoomsPercentage($roomId, $startDate, $endDate)
    {
        ...
        $totalRoomsbase = $room->number;
        $startDat = Carbon::parse($startDate);
        $endDat = Carbon::parse($endDate);

        $roomsFromDateTable = HotelRoomDate::where('target_id', $roomId)
            ->where(function ($query) use ($startDat, $endDat) {
                $query->whereDate('start_date', '<=', $endDat)
                    ->whereDate('end_date', '>=', $startDat);
            })
            ->sum('number');

        $bookedRooms = HotelRoomBooking::where('room_id', $roomId)
            ->where(function ($query) use ($startDat, $endDat) {
                $query->whereDate('start_date', '<=', $endDat)
                    ->whereDate('end_date', '>=', $startDat);
            })
        ...
        if ($roomsFromDateTable === $totalRoomsbase) {

            $availableRooms = $totalRoomsbase - $bookedRooms;
            $totalRooms = $roomsFromDateTable;
        } elseif ($roomsFromDateTable !== $totalRoomsbase && $roomsFromDateTable > 0) {

            $availableRooms = $roomsFromDateTable - $bookedRooms;
            $totalRooms =$roomsFromDateTable;
        } else {

            $availableRooms = $totalRoomsbase - $bookedRooms;
            $totalRooms = $totalRoomsbase;
        }

        $availableRooms = max($availableRooms, 0);
        $totalRooms = max($totalRooms, 0);

        $percentageAvailable = ($totalRoomsbase > 0) ? ($availableRooms / $totalRooms) * 100 : 100;

        return round($percentageAvailable, 1);
    }

        public function initSettings($vendorId ,$hotelId ,$roomId, $startDate, $endDate) {

            $vendorId = User::find($vendorId);
            $hotelId = Hotel::find($hotelId);
            // $filePath = 'rooms_availability.json';
            // file_put_contents($filePath, json_encode($role, JSON_PRETTY_PRINT));

            if (!empty($vendorId) && !empty($hotelId)) {

            // commission_type
            if($hotelId->hotel_commission_type != "default" && $hotelId->hotel_commission_type !== null){
                $this->commissionType = $hotelId->hotel_commission_type;
            }
            elseif($vendorId->vendor_commission_type != "default" && $vendorId->vendor_commission_type !== null){
                $this->commissionType = $vendorId->vendor_commission_type;
            }
            else{
                $this->commissionType = setting_item('vendor_commission_type');
            }

            //commission_amount

            if ($hotelId->hotel_commission_amount) {
                $defaultCommission = $hotelId->hotel_commission_amount;
            } elseif ($vendorId->vendor_commission_amount) {
                $defaultCommission = $vendorId->vendor_commission_amount;
            } else {
                $defaultCommission = (float) setting_item('vendor_commission_amount');
            }

            $availabilityPercentage = $this->getAvailableRoomsPercentage($roomId, $startDate, $endDate);

            $settings = json_decode(setting_item('vendor_commission_availability_type'), true);

            usort($settings, fn($a, $b) => floatval($b['availability_target']) <=> floatval($a['availability_target']));

            $commissionValue = null;

            if (is_array($settings)) {
                foreach ($settings as $key => $setting) {
                    if (isset($setting['availability_target'], $setting['vendor_commission_value'])) {
                        $currentTarget = (double)$setting['availability_target'];
                        $nextTarget = isset($settings[$key + 1]) ? (double)$settings[$key + 1]['availability_target'] : null;
                        if ($availabilityPercentage <= $currentTarget &&
                            (is_null($nextTarget) || $availabilityPercentage > $nextTarget)) {
                            $commissionValue = (float)$setting['vendor_commission_value'];
                            break;
                        }
                    }
                }
            ...
            $this->commissionAmount = $commissionValue ?? $defaultCommission;


            //commission_calculate_time
            if($hotelId->hotel_commission_calculate_time != "default" && $hotelId->hotel_commission_calculate_time !== null){
                $this->commissionTime = $hotelId->hotel_commission_calculate_time;
            }
            elseif($vendorId->vendor_commission_calculate_time != "default" && $vendorId->vendor_commission_calculate_time !== null){
                $this->commissionTime = $vendorId->vendor_commission_calculate_time;
            }
            else{
                $this->commissionTime = setting_item('vendor_commission_calculate_time');
            }

            //commission_calculate_way
            if($hotelId->hotel_commission_calculate_way != "default" && $hotelId->hotel_commission_calculate_way !== null){
                $this->commissionWay = $hotelId->hotel_commission_calculate_way;
            }
            elseif($vendorId->vendor_commission_calculate_way != "default" && $vendorId->vendor_commission_calculate_way !== null){
                $this->commissionWay = $vendorId->vendor_commission_calculate_way;
            }
            else{
                $this->commissionWay = setting_item('vendor_commission_calculate_way');
            }

            //commission_per_person
            if($hotelId->hotel_per_person != "default" && $hotelId->hotel_per_person !== null){
                $this->perPerson = $hotelId->hotel_per_person;
            }
            elseif($vendorId->per_person != "default" && $vendorId->per_person !== null){
                $this->perPerson = $vendorId->per_person;
            }
            else{
                $this->perPerson = (int) setting_item('per_person');
            }


            if($vendorId->vendor_commission_type == 'disable' || $hotelId->vendor_commission_type == 'disable'){
                return;
            }
            }
        ...
        public function CalculateCommissionAddition($total, $adults, $numberDays,$vendorId,$hotelId,$roomId, $startDate, $endDate) {
            if ($this->commissionType === null) {
                $this->initSettings($vendorId,$hotelId,$roomId, $startDate, $endDate);
            }
            ...
            if (setting_item('vendor_enable') == 1) {
                ...
                if ($this->commissionWay === 'addition') {
                    if ($this->commissionType === 'amount') {
                        if ($this->commissionTime === 'one-time' && !$this->perPerson) {
                            $totalCommission = $this->commissionAmount;
                        } elseif ($this->commissionTime === 'per-day' && !$this->perPerson) {
                            $totalCommission = $numberDays * $this->commissionAmount;
                        }
                    }

                    if ($this->commissionType === 'percent') {
                            $totalCommissionvalue = ($total * $this->commissionAmount) / 100;
                            if ($this->commissionTime === 'one-time') {
                                $totalCommission = $totalCommissionvalue / $numberDays;
                            }elseif ($this->commissionTime === 'per-day') {
        ...
        public function CalculateCommissionAdditionperdayandperson($total, $adults, $numberDays,$vendorId,$hotelId, $roomId, $startDate, $endDate) {
            if ($this->commissionType === null) {
                $this->initSettings($vendorId,$hotelId,$roomId, $startDate, $endDate);
            }
            ...
            if (setting_item('vendor_enable') == 1) {
                ...
                if ($this->commissionWay === 'addition') {
                    if ($this->commissionType === 'amount') {
                        if($this->commissionTime === 'one-time' && $this->perPerson){
                            $totalCommission = $adults * $this->commissionAmount;
                        }
                        elseif ($this->commissionTime === 'per-day' && $this->perPerson) {
                            $totalCommission = ($numberDays * $adults) * $this->commissionAmount;
                        }
                    }
                    // elseif ($this->commissionType === 'percent') {
                    //     $commissionpercent = ($total * $this->commissionAmount) / 100;
                    //         if($this->commissionTime === 'one-time' && $this->perPerson){
                    //             $totalCommission = $adults * ($commissionpercent/$numberDays);
                    //         }
                    //         elseif ($this->commissionTime === 'per-day' && $this->perPerson) {
                    //             $totalCommission =$adults * $commissionpercent ;
                    //         }
                    // }
                }
                return $totalCommission;
        ...
        public function CalculateCommissionDediction($total, $adults, $numberDays,$vendorId,$hotelId,$roomId, $startDate, $endDate) {
            if ($this->commissionType === null) {
                $this->initSettings($vendorId,$hotelId,$roomId, $startDate, $endDate);
            }
            if (setting_item('vendor_enable') == 1) {
                $totalCommission = 0;
                if ($this->commissionWay === 'dedict') {
                    if ($this->commissionType === 'amount') {
                        if ($this->commissionTime === 'one-time' && !$this->perPerson) {
                            $totalCommission = $this->commissionAmount;
                        }
                        ...
                        elseif ($this->commissionTime === 'one-time' && $this->perPerson) {
                            $totalCommission = $adults * $this->commissionAmount;
                        }
                        elseif ($this->commissionTime === 'per-day' && $this->perPerson) {
                            $totalCommission = ($numberDays * $adults) * $this->commissionAmount;
    ...
    public function getCommissionVendor($rooms,$startDate,$endDate){

        $vendorId = $this->vendor_id;
        $hotelId = $this->object_id;
        foreach ($rooms as $room) {
            $room_id = $room['room_id'];
        if ($this->commissionType === null) {
            $this->initSettings($vendorId,$hotelId,$room_id, $startDate, $endDate);
        }
        }
        $total = $this->total_before_fees;
        $total_guest=$this->total_guests;
        $numberDay = $this->getDurationDaysAttribute()-1;

        $returnArray=[
            'commission'=>0,
            'commission_type'=>'',
        ];
        $totalCommissionaddition = 0;
        $totalCommissiondediction = 0;
        $totalCommissionpercent = 0;

        if($this->commissionType === 'percent'){
        foreach ($rooms as $room) {
            $room_id = $room['room_id'];
            $numberSelected = $room['number_selected'];
            $bookingData = HotelRoom::where('id', $room_id)
            ->select('price')
            ->first();
            $roompricebeforbooking=$bookingData->price;
                    if ($this->commissionTime === 'one-time') {
                        $totalCommissionpercents = (($roompricebeforbooking * $this->commissionAmount) / 100) * $numberSelected;
                        if($this->commissionTime === 'one-time'){
                            $commissionDay =(($roompricebeforbooking * $this->commissionAmount) / 100) * $numberSelected;
                            $totalCommissionpercents =1 * $commissionDay;
                        }
                    }
                    elseif ($this->commissionTime === 'per-day') {
                        $totalCommissionpercents =(($roompricebeforbooking * $this->commissionAmount) / 100) * $numberDay * $numberSelected;
                        if($this->commissionTime === 'per-day'){
                            $commissionDay =(($roompricebeforbooking * $this->commissionAmount) / 100) * $numberSelected;
                            $totalCommissionpercents =$numberDay * 1* $commissionDay;
                        }
                    }
                    $totalCommissionpercent +=$totalCommissionpercents;
        }}
        ...
        foreach ($rooms as $room) {
            ...
            if($this->commissionWay === 'addition'){
                if(!$this->perPerson ){
                    $roomCommission = $this->CalculateCommissionAddition($roomPrice, $total_guest, $numberDay, $vendorId, $hotelId, $roomId, $startDate, $endDate) * $numberSelected;
                }
                else{
                    $roomCommission = $this->CalculateCommissionAdditionperdayandperson($roomPrice, $total_guest, $numberDay, $vendorId, $hotelId, $roomId, $startDate, $endDate);
                }
                $totalCommissionaddition += $roomCommission;
            }
            else{
                if(!$this->perPerson ){
                    $roomCommission = $this->CalculateCommissionDediction($roomPrice, $total_guest, $numberDay, $vendorId, $hotelId, $roomId, $startDate, $endDate) * $numberSelected;
                }
                else{
                    $roomCommission = $this->CalculateCommissionDediction($roomPrice, $total_guest, $numberDay, $vendorId, $hotelId, $roomId, $startDate, $endDate);
                }
                $totalCommissiondediction += $roomCommission;
            }
        ...
        $commissiontypes['amount']=$this->commissionAmount;
        $commissiontypes['type']=$this->commissionType;
        $commissiontypes['time']=$this->commissionTime;
        $commissiontypes['way']=$this->commissionWay;
        $commissiontypes['per_person']=$this->perPerson;

                if ($this->commissionType === 'percent') {
                    if($this->commissionWay ==='addition'){
                        $returnArray['commission']=$totalCommissionpercent;
                    }
                    else{
                        $returnArray['commission']=$totalCommissionpercent;
                    }
                }
                elseif($this->commissionType === 'amount') {
                    if($this->commissionWay ==='addition'){
                        $returnArray['commission']=$totalCommissionaddition;
                    }
                    else{
                        $returnArray['commission']=$totalCommissiondediction;
                    }
                }
                $returnArray['commission_type'] = json_encode($commissiontypes);


        return $returnArray;
    }

    public function calculateCommission($rooms,$startDate,$endDate)
    {
        $data = $this->getCommissionVendor($rooms,$startDate,$endDate);
    ...
    public function reloadCalculateTotalBooking(){
        ...
        $total_service_fee = 0;
        if(!empty($list_fees = $vendorId_service_fee)){
        ...
        $vendorId_service_fee_amount = $total_service_fee;
