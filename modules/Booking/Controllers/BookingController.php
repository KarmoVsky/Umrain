...
class BookingController extends \App\Http\Controllers\Controller
{
    ...
    public function doCheckout(Request $request)
    {
        ...
        if ($this->countryHasStates($country_id)) {
            $rules['state'] = 'required';
        }
        $state_id = $request->input('state');
        if ($this->stateHasCities($state_id)) {
            $rules['city'] = 'required';
        }
...
//        event(new VendorLogPayment($booking));
    ...
    private function countryHasStates($code)
    {
    ...
    private function stateHasCities($state_name)
    {
        ...
        if ($totalPassenger = $booking->calTotalPassenger()) {
            ...
            for ($i = 1; $i <= $totalPassenger; $i++) {
                ...
                $data = [
                    ...
                    'email'      => $input[$i]['email'] ?? '',
                    'phone'      => $input[$i]['phone'] ?? '',
                    'country_code'=>$input[$i]['country_code'] ?? '',
                ];
    ...
    public function addEnquiry(Request $request)
    {
        ...
        $row->fill([
            ...
            'country_code'=>$request->input('enquiry_country_code'),
