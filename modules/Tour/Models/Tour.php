...
class Tour extends Bookable
{
    ...
    /*
        This Variable ($invoice_new_booking_file) has the same value of ($email_new_booking_file) temporarly
        when design newinvoice for Tour must change its value for the path of new design
    */
    public    $invoice_new_booking_file             = 'Tour::emails.new_booking_detail';
    public    $type                               = 'tour';
    ...
    public function search($request)
    {
        ...
        $model_Tour->where(function ($query) {
        ...
        if (!empty($request['start']) && !empty($request['end'])) {
            ...
            $model_Tour->where(function($q) use ($start_date, $end_date, $request) {
                $q->where('default_state', 1);

                $q->orWhere(function ($qu) use ($start_date, $end_date) {
                    $qu->where('default_state', 0)
                        ->whereHas('tour_dates', function($subQuery) use ($start_date, $end_date) {
                        ...
                        ->where(function($e) use ($request) {
                            if (isset($request['location_id'])) {
                                $location = Location::where('id', $request['location_id'])->where('status', 'publish')->first();
                                $locationId = $location ? $location->parent_id : null;

                                if($locationId != null) {
                                    $e->where('location_id', $request['location_id']);
                                } else {
                                    $childrenLocations = Location::where('parent_id', $request['location_id'])->pluck('id');
                                    if ($childrenLocations->isNotEmpty()) {
                                        $e->whereIn('location_id', $childrenLocations);
                                    }
                                }
                            }
                            else {
                                $activeLocationIds = Location::where('status', 'publish')
                                    ->whereHas('tours', function($q) {
                                        $q->where('status', 'publish');
                                    })
                                    ->pluck('id');
                                $e->whereIn('location_id', $activeLocationIds);
                            }
                        });
            });
        }
    ...
    public function shouldDisplayVendorInfo()
    {
        ...
        if ($vendor->is_visible && $this->vendor_override_show_name === 'show') {
            return true;
        }

        if (!$vendor->is_visible && $this->vendor_override_show_name==='hidden') {
            return false;
        }
        ...
        if ($vendor->is_visible && $this->vendor_override_show_name==='by_default') {
            return true;
        }
        if (!$vendor->is_visible && $this->vendor_override_show_name==='by_default') {
            return false;
    ...
    {
        return $this->hasMany(TourDate::class, "target_id");
    }

    public function acceptedVendors() {
        return $this->hasMany(\App\Models\BusinessRelation::class, 'service_id')
                    ->where('service_type', 'tour')->whereIn('status', ['approved', 'draft']);
    }
