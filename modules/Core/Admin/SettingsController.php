    ...
    public function store(Request $request, $group)
    {
        ...
        if (!empty($request->input())) {
            if (!empty($keys)) {
                ...
                if (!empty($message = $request->disable_message)) {
                    $module = Module::where('name', $group)->first();
                    $languages = \Modules\Language\Models\Language::getActive();

                    if ($module) {
                        $currentMessages = json_decode($module->message, true);
                        $nav_lang = request()->lang? request()->lang : setting_item('site_locale');

                        foreach ($languages as $lang) {
                            $lang = $lang['locale'];
                            if($lang == $nav_lang) {
                                $currentMessages['name_'.$lang] = $message['name_'.$nav_lang];
                            }
                        }
                        $module->update([
                            'message' => json_encode($currentMessages),
                        ]);
                    } else {
                        $messages = [];
                        $nav_lang = request()->lang? request()->lang : setting_item('site_locale');
                        foreach($languages as $lang) {
                            $lang = $lang['locale'];
                            if($lang == $nav_lang) {
                                $messages['name_'.$lang] = $message['name_'.$nav_lang];
                            } else {
                                $messages['name_'.$lang] = null;
                            }
                        }
                        Module::create([
                            'name' => $group,
