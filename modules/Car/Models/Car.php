...
class Car extends Bookable
{
    ...
    /*
        ...
        when design newinvoice for Car must change its value for the path of new design
    */
    public $invoice_new_booking_file             = 'Car::emails.new_booking_detail';
    ...
    public function search($request)
    {
        ...
        if (!empty($request['start']) && !empty($request['end'])) {
            ...
            //------------------------
            $query->where(function($q) use ($start_date, $end_date, $request) {
                $q->where('default_state', 1);

                $q->orWhere(function ($q) use ($start_date, $end_date) {
                    $q->where('default_state', 0)
                    ->whereHas('car_dates', function($subQuery) use ($start_date, $end_date) {
    ...
    {
        return $this->hasMany(CarDate::class, 'target_id');
    }

    public function acceptedVendors() {
        return $this->hasMany(\App\Models\BusinessRelation::class, 'service_id')
                    ->where('service_type', 'car')->whereIn('status', ['approved', 'draft']);
    }
}
